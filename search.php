...
if ($hasSearch) {
    ...
    if ($filterType === 'all' || $filterType === 'hotel_voucher') {
        $hotelVouchers = Database::getInstance()->fetchAll(
    ...
    if ($filterType === 'all' || $filterType === 'tour_voucher') {
        $tourVouchers = Database::getInstance()->fetchAll(
    ...
    if ($filterType === 'all' || $filterType === 'company') {
        $companies = Database::getInstance()->fetchAll(
...
$companies = Database::getInstance()->fetchAll("SELECT DISTINCT company as name FROM partners WHERE status = 'active' ORDER BY company");
    ...
    <div class="page-header-content">
        <div>
            ...
            <p class="page-subtitle">
                ...
                <?php else: ?>"<?php echo htmlspecialchars($query ?: 'Tum'); ?>" icin <?php echo htmlspecialchars(count($results)); ?> sonuc bulundu
                <?php endif; ?>
            </p>
        </div>
    </div>
...
<div style="display: flex; gap: 0.5rem; margin-bottom: 1.5rem; flex-wrap: wrap;">
    <a href="?<?php echo http_build_query(array_merge($_GET, ['type' => 'all'])); ?>" 
       class="btn btn-sm <?php echo $filterType === 'all' ? 'btn-primary' : 'btn-secondary'; ?>">
        Tumu (<?php echo htmlspecialchars(count($results)); ?>)
    </a>
    <?php
    $typeCounts = [];
...
<div class="grid grid-cols-2">
    ...
    ?>
    <div class="card search-result" data-type="<?php echo htmlspecialchars($result['type']); ?>">
        <div class="card-body">
            <div style="display: flex; align-items: flex-start; gap: 1rem;">
                ...
                <div style="flex: 1;">
                    <div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.25rem;">
                        ...
                        <span class="badge badge-<?php echo htmlspecialchars($result['status']) === 'paid' ? 'success' : 'warning'; ?>">
                    ...
                    <p style="font-size: 0.75rem; color: var(--gray-400);">
                        ...
                        <i class="fas fa-users" style="margin: 0 0.25rem 0 0.5rem;"></i><?php echo htmlspecialchars($result['total_pax']); ?>
                        <?php endif; ?>
                    </p>
                </div>
