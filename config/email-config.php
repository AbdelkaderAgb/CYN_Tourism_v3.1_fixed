...
// Load current config
$config = Database::getInstance()->fetchOne("SELECT * FROM email_config LIMIT 1") ?? [];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Validate CSRF token
    if (!isset($_POST['cyn_csrf_token']) || !hash_equals($_SESSION['cyn_csrf_token'] ?? '', $_POST['cyn_csrf_token'])) {
