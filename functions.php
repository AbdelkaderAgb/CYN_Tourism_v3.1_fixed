...
// Ensure required constants are defined (fallback if config.php not loaded)
if (!defined('DATE_FORMAT')) define('DATE_FORMAT', 'd/m/Y');
if (!defined('DATETIME_FORMAT')) define('DATETIME_FORMAT', 'd/m/Y H:i');
if (!defined('DEFAULT_ITEMS_PER_PAGE')) define('DEFAULT_ITEMS_PER_PAGE', 20);
if (!defined('MAX_ITEMS_PER_PAGE')) define('MAX_ITEMS_PER_PAGE', 100);
if (!defined('PAGINATION_RANGE')) define('PAGINATION_RANGE', 2);
if (!defined('CSRF_TOKEN_NAME')) define('CSRF_TOKEN_NAME', 'cyn_csrf_token');
...
function get_dashboard_stats() {
    ...
    try {
        // Today's transfers
        $today = Database::getInstance()->fetchOne(
        ...
        // Monthly vouchers
        $monthVouchers = Database::getInstance()->fetchOne(
        ...
        // Total partners
        $partners = Database::getInstance()->fetchOne("SELECT COUNT(*) as count FROM partners WHERE status = 'active'");
        $stats['total_partners'] = (int) ($partners['count'] ?? 0);
        
        // Total vehicles
        $vehicles = Database::getInstance()->fetchOne("SELECT COUNT(*) as count FROM vehicles WHERE status = 'active'");
        $stats['total_vehicles'] = (int) ($vehicles['count'] ?? 0);
        
        // Total drivers
        $drivers = Database::getInstance()->fetchOne("SELECT COUNT(*) as count FROM drivers WHERE status = 'active'");
